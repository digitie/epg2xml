            ...
            try:
                ...
                for program in data['Programs']:
                    ...
                    if program['Channel'] == "CHAM" and program['LiveDays'] == dayofweek[day.weekday()]:
                        ...
                        matches = re.match(pattern, unescape(program['ProgramTitle']))
                        if not(matches is None):
        ...
        try:
            ...
            try:
                ...
                for program in data['resultList']:
                    ...
                    matches = re.match(pattern, unescape(program['program_title']))
                    if not(matches is None):
                        programName = matches.group(1)
                        rebroadcast = True if matches.group(2) else False
        ...
        try:
            ...
            try:
                ...
                for row in soup.find_all('li'):
                    ...
                    matches = re.match(pattern, unescape(row.text))
                    if not(matches is None):
        ...
        try:
            ...
            if(html):
                for row in html:
                    ...
                    matches = re.match(pattern, unescape(row.find('td').text.decode('string_escape').strip()))
                    if not(matches is None):
