...
CONTENT_ERROR = 'Invalid EPG'
HTTP_ERROR = 'EPG Retrieving error'
SOCKET_ERROR = 'xmltv.sock not found'
JSON_FILE_ERROR = 'json not found'
JSON_SYNTAX_ERROR = 'Invalid json'
...
def getEpg():
    ...
    ChannelInfos = []
    try:
        with open(Channelfile, encoding='utf8') as f: # Read Channel Information file
    ...
    for Channeldata in Channeldatas: #Get Channel & Print Channel info
        if Channeldata['Enabled'] == 1:
            ...
            print('    <display-name>{0}</display-name>'.format(ChannelName))
        ...
        try:
            ...
            if(html):
                ...
                for epg1, epg2 in zip(epginfo, epginfo[1:]):
                    ...
                    subprogrmaName = ''
                    print(epg1[0])
                    #matches = re.match('^(.*?)( <(.*)>)?$', epg1[0].decode('unicode_escape'))
                    matches = re.match('^(.*?)( <(.*)>)?$', epg1[0])
                    if not (matches is None):
        ...
        try:
            ...
            if(html):
                ...
                for epg1, epg2 in zip(epginfo, epginfo[1:]):
                    ...
                    matches = re.match('(<재>?)?(.*?)(\[(.*)\])?\s?(\(([\d,]+)회\))?$',  epg1[0])
                    rebroadcast = False
                    if not (matches is None):
        ...
        try:
            ...
            if (data['channel'] is None) :
                ...
                for program in programs:
                    ...
                    episode = ''
                    rebroadcast = False
                    matches = re.match('^(.*?)(?:\s*[\(<]([\d,회]+)[\)>])?(?:\s*<([^<]*?)>)?(\((재)\))?$', program['programName'].replace('...', '>'))
                    if not (matches is None):
...
ChannelInfos = []
try:
    with open(Settingfile, encoding='utf8') as f: # Read Channel Information file
